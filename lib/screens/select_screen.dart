...
class SelectScreen extends StatelessWidget {
        ...
        children: [
          ...
          if (count > 0) ...[
            ...
            Row(
              ...
              children: [
                _buildHeaderChip('全选', Icons.select_all_rounded, state),
                const SizedBox(width: 8),
                _buildHeaderChip('清空', Icons.close_rounded, state),
              ],
            ),
          ],
        ],
  ...
  Widget _buildHeaderChip(String label, IconData icon, AppState state) {
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.photo_library_outlined, size: 72, color: AppTheme.primaryLight),
            ),
            const SizedBox(height: 20),
            const Text('还没有加载照片',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.w600, color: AppTheme.textPrimary)),
            const SizedBox(height: 8),
            const Text('请允许访问相册来开始制作',
  ...
  Widget _buildYearBar(AppState state, List<int> years) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: AppTheme.primaryColor.withAlpha(8), blurRadius: 4, offset: const Offset(0, 2)),
        ],
      ),
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          if (index == 0) {
            ...
            return _buildYearChip(isSelected, state, null, state.allPhotos.length);
          }
          final year = years[index - 1];
          final isSelected = year == state.selectedYear;
          final count = state.photosByYear[year]?.length ?? 0;
          return _buildYearChip(isSelected, state, year, count);
        },
      ),
    );
  }

  Widget _buildYearChip(bool isSelected, AppState state, int? year, int count) {
    return Padding(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: isSelected
                ? [BoxShadow(color: AppTheme.primaryColor.withAlpha(40), blurRadius: 8, offset: const Offset(0, 3))]
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: const Text('✨  开始排版', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
                ),
              ),
              const SizedBox(height: 8),
            ],
